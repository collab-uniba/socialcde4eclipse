...
public class ActionInteractiveTimeline {
	...
	private Image getUserImage(String link) {
		try {
			...
			return resize(get_ImageStream(this.getClass().getClassLoader()
					.getResourceAsStream("images/DefaultAvatar.png")), 48, 48);
	...
	public ActionInteractiveTimeline(final HashMap<String, Object> uiData) {
		...
		case "usernameLink":
			...
			if (Controller.getProxy().IsWebServiceRunning()) {
				...
				Controller
						.openConnectionLostPanel("Connection Lost! \n You will be redirected to Login page.");
			}
			break;
		...
		case "otherPostAvailable":
			if (Controller.getProxy().IsWebServiceRunning()) {
				...
				final WPost[] posts = Controller.getProxy()
						.GetInteractiveTimeline(
								Controller.getCurrentUser().Username,
								Controller.getCurrentUserPassword(), "",
								DynamicInteractiveTimeline.getFileSelected(),
								"File", 0, getLastId());

				if (posts.length > 0) {
					for (Control element : ((Composite) uiData
							.get("userPostMaster")).getChildren()) {
						try {
				...
				for (int i = 0; i < posts.length; i++) {
					...
					Display.getCurrent().syncExec(new Runnable() {
						...
						public void run() {
							...
							if (((ProgressBar) uiData.get("pbar"))
									.getSelection() == (max - 1)) {
								((ProgressBar) uiData.get("pbar"))
										.setSelection(0);
							} else {
								((ProgressBar) uiData.get("pbar")).setSelection(((ProgressBar) uiData
							...
							userPostComposite.setBackground(Display
							...
							Label labelUserAvatar = new Label(
									userPostComposite, SWT.NONE);
							gridData = new GridData();
							gridData.verticalSpan = 3;
							gridData.verticalAlignment = SWT.BEGINNING;
							labelUserAvatar.setLayoutData(gridData);

							if (Controller.getUsersAvatar().get(
									posts[j].getUser().Username) == null) {
								Controller
										.getUsersAvatar()
										.put(posts[j].getUser().Username,
												getUserImage(posts[j].getUser().Avatar));
							}
							labelUserAvatar.setImage(resize(
									new Image(Display.getCurrent(), Controller
											.getUsersAvatar()
											.get(posts[j].getUser().Username),
											SWT.IMAGE_COPY), 48, 48));
							...
							if (!posts[j].getUser().Username.equals(Controller
								...
								labelUserAvatar.setData("ID_action",
										"labelAvatarLink");
								labelUserAvatar.addListener(SWT.MouseDown,
										azioni);
							}

							Label username = new Label(userPostComposite,
									SWT.None);
							...
							if (!posts[j].getUser().Username.equals(Controller
								...
								username.setCursor(new Cursor(Display
										.getCurrent(), SWT.CURSOR_HAND));
								username.setToolTipText("View "
										+ posts[j].getUser().Username
										+ " Timeline");
								username.setData("User_data",
										posts[j].getUser());
							...
							message.setBackground(Display.getCurrent()
							...
							if (days > 1 && days < 30) {
								...
								messageDate
										.setText("More than one month ago from "
												+ posts[j].getService()
														.getName());
							} else if (days == 1) {
								messageDate.setText("About " + days
					...
					// System.out.println("getLastId aggiornamento " +
					// getLastId());
				...
				WPost[] newPosts = Controller.getProxy()
						.GetInteractiveTimeline(
								Controller.getCurrentUser().Username,
								Controller.getCurrentUserPassword(), "",
								DynamicInteractiveTimeline.getFileSelected(),
				...
				otherPostWarning.setBackground(Display.getCurrent()
						.getSystemColor(SWT.COLOR_WHITE));
				GridData gridData = new GridData();
				gridData.horizontalSpan = 2;
				gridData.grabExcessHorizontalSpace = true;
				...
				if (newPosts.length > 0) {
					Link otherPostAvailable = new Link(otherPostWarning,
							SWT.NONE);
					otherPostAvailable.setCursor(new Cursor(Display
							.getCurrent(), SWT.CURSOR_HAND));
					otherPostAvailable.setFont(new Font(Controller.getWindow()
							.getDisplay(), "Calibri", 10, SWT.UNDERLINE_LINK));
					otherPostAvailable
							.setText("<a>Click to view older posts</a>");
					...
					otherPostAvailable.setData("ID_action",
							"otherPostAvailable");

				} else {
					Label noPostAvailable = new Label(otherPostWarning,
							SWT.NONE);
					noPostAvailable
							.setText("There are no post in the cache.\n Please try again in two minutes.");
				...
				((ScrolledComposite) uiData.get("superUserPostMaster"))
						.layout();
				((ScrolledComposite) uiData.get("superUserPostMaster"))
						.redraw();
			...
			if (Controller.getProxy().IsWebServiceRunning()) {
				...
				if (!InterceptingFilter.verifyText(((Text) uiData
					...
					if (!Controller.getProxy().Post(
						...
						WPost[] interactiveTimelinePost = Controller.getProxy()
								.GetUserTimeline(
										Controller.getCurrentUser().Username,
										Controller.getCurrentUserPassword(),
										Controller.getCurrentUser().Username);
						...
						userPostComposite.setData("IdPost",
								interactiveTimelinePost[0].Id);
						...
						if (Controller.getUsersAvatar().get(
								Controller.getCurrentUser().Username) == null) {
							Controller.getUsersAvatar()
									.put(Controller.getCurrentUser().Username,
											getUserImage(Controller
						...
						// reading message unescaping the text for correct
						// visualization
						message.setText(findLink(StringEscapeUtils
								.unescapeJava(userMessage)));
						...
						messageDate.setForeground(new Color(Display
								.getCurrent(), 140, 140, 140));
						messageDate
								.setText("About one minutes ago from SocialTFS");
						...
						/*
						 * Label labelhidden = new Label( ((Composite)
						 * uiData.get("userPostMaster")), SWT.None);
						 * labelhidden.setText("prova");
						 * labelhidden.setVisible(false);
						 */
						Label barSeparator = new Label(userPostComposite,
