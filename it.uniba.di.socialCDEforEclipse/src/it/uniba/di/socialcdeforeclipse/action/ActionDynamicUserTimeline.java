	...
	public ActionDynamicUserTimeline(final HashMap<String, Object> uiData) {
		...
		case "labelHide":
			...
			hideUserSetting.setyCoordinate(Controller.getWindow().toDisplay(
					...
					Controller.getWindow().getLocation().y).y);
			hideUserSetting
					.setxCoordinateWithOffset(Controller.getWindow().toDisplay(
							Controller.getWindow().getLocation().x,
							Controller.getWindow().getLocation().y).x
							- 30
		...
		case "otherPostAvailable":
			 ...
			 * Display.getCurrent().update(); ((ProgressBar)
			 * uiData.get("pbar")).setVisible(true);
			 * 
			 * 
			 * 
			 * final WPost[] posts =
			 * Controller.getProxy().GetUserTimeline(Controller
			 * .getCurrentUser().Username, Controller.getCurrentUserPassword(),
			 * ( (WUser) uiData.get("userSelected") ).Username,0,getLastId());
			 * 
			 * final int max = 100; for(int i=0;i< posts.length; i++) {
			 * 
			 * 
			 * final Composite userPostComposite = new Composite(((Composite)
			 * uiData.get("userPostMaster")), SWT.None); final int j=i;
			 * 
			 * Display.getCurrent().syncExec(new Runnable() {
			 * 
			 * @Override public void run() { // TODO Auto-generated method stub
			 * 
			 * if (((ProgressBar) uiData.get("pbar")).getSelection() == (max -
			 * 1)) { ((ProgressBar) uiData.get("pbar")).setSelection(0); } else
			 * { ((ProgressBar) uiData.get("pbar")).setSelection(((ProgressBar)
			 * uiData.get("pbar")).getSelection() + 1); ((ProgressBar)
			 * uiData.get("pbar")).redraw(); //Display.getCurrent().update(); }
			 * 
			 * userPostComposite.setLayout(new GridLayout(2, false)); GridData
			 * gridData = new GridData(); gridData.grabExcessHorizontalSpace =
			 * true; gridData.horizontalAlignment = GridData.FILL;
			 * userPostComposite.setLayoutData(gridData);
			 * 
			 * Label labelUserAvatar = new Label(userPostComposite,SWT.NONE);
			 * gridData = new GridData(); gridData.verticalSpan = 3;
			 * labelUserAvatar.setLayoutData(gridData);
			 * labelUserAvatar.setData("ID_action", "labelAvatar");
			 * 
			 * 
			 * try { labelUserAvatar.setImage(get_ImageStream(new URL(
			 * posts[j].getUser().Avatar).openStream()));
			 * labelUserAvatar.setImage(resize(labelUserAvatar.getImage(), 75,
			 * 75));
			 * 
			 * } catch (IOException e) { // TODO Auto-generated catch block
			 * //System.out.println("Eccezione lanciata");
			 * labelUserAvatar.setImage
			 * (get_ImageStream(this.getClass().getClassLoader
			 * ().getResourceAsStream("images/DefaultAvatar.png")));
			 * labelUserAvatar.setImage(resize(labelUserAvatar.getImage(), 75,
			 * 75)); //e.printStackTrace(); } catch (NullPointerException e) {
			 * // TODO: handle exception
			 * labelUserAvatar.setImage(get_ImageStream
			 ...
			 * 97,91,91)); username.setFont(new
			 * Font(Controller.getWindow().getDisplay(),"Calibri", 15, SWT.BOLD
			 * )); gridData = new GridData(); gridData.grabExcessHorizontalSpace
			 * = false; gridData.horizontalAlignment = GridData.FILL;
			 * username.setLayoutData(gridData);
			 * 
			 * Label message = new Label(userPostComposite, SWT.None |
			 * SWT.WRAP); message.setText(posts[j].getMessage()); gridData = new
			 * GridData(); gridData.widthHint = 400;
			 * message.setLayoutData(gridData);
			 * 
			 * Calendar nowDate = Calendar.getInstance(); Calendar dateSelected
			 * = posts[j].getCreateAt(); long millisDiff =
			 * nowDate.getTime().getTime() - dateSelected.getTime().getTime();
			 * 
			 * int seconds = (int) (millisDiff / 1000 % 60); int minutes = (int)
			 * (millisDiff / 60000 % 60); int hours = (int) (millisDiff /
			 * 3600000 % 24); int days = (int) (millisDiff / 86400000);
			 * 
			 * Label messageDate = new Label(userPostComposite, SWT.None);
			 * messageDate.setForeground(new Color(Display.getCurrent(),
			 * 140,140,140)); if(days > 1 && days < 30) {
			 * messageDate.setText("About " + days + " days ago from " +
			 * posts[j].getService().getName()); } else if(days > 30) {
			 * messageDate.setText("More than one month ago from " +
			 * posts[j].getService().getName()); } else if(days == 1) {
			 * messageDate.setText("About " + days + " day ago from " +
			 * posts[j].getService().getName()); } else { if( hours > 1) {
			 * messageDate.setText("About " + hours + " hours ago from " +
			 * posts[j].getService().getName()); } else if(hours == 1) {
			 * messageDate.setText("About " + hours + " hour ago from " +
			 * posts[j].getService().getName()); } else {
			 * 
			 * if( minutes > 1) { messageDate.setText("About " + minutes +
			 * " minutes ago from " + posts[j].getService().getName()); } else
			 * if(minutes == 1) { messageDate.setText("About " + minutes +
			 * " minute ago from " + posts[j].getService().getName()); } else {
			 * 
			 * if( seconds > 1) { messageDate.setText("About " + seconds +
			 * " seconds ago from " + posts[j].getService().getName()); } else
			 * if(seconds == 1) { messageDate.setText("About " + seconds +
			 * " second ago from " + posts[j].getService().getName()); } else {
			 * messageDate.setText("Few seconds ago from " +
			 * posts[j].getService().getName()); } } } }
			 * 
			 * messageDate.setFont(new
			 * Font(Controller.getWindow().getDisplay(),"Calibri", 8, SWT.ITALIC
			 * )); gridData = new GridData(); gridData.horizontalAlignment =
			 * GridData.BEGINNING; messageDate.setLayoutData(gridData);
			 * 
			 * Label barSeparator = new Label(((Composite)
			 * uiData.get("userPostMaster")), SWT.BORDER); gridData = new
			 * GridData(); gridData.widthHint = 100; gridData.heightHint = 1;
			 ...
			 * 
			 * Controller.setScrollHeight(Controller.getWindowHeight() + (80 *
			 * ((Composite) uiData.get("userPostMaster")).getChildren().length)
			 * ); ((ScrolledComposite)
			 * Controller.getWindow().getParent()).setMinSize
			 * (Controller.getWindowWidth()-50, Controller.getScrollHeight());
			 * 
			 * WPost[] newPosts =
			 * Controller.getProxy().GetUserTimeline(Controller
			 * .getCurrentUser().Username, Controller.getCurrentUserPassword(),
			 ...
			 * Label noPostAvailable = new Label(((Composite)
			 * uiData.get("otherPostWarning")),SWT.NONE);
			 * noPostAvailable.setText("There are no older post available.");
			 * noPostAvailable.setFont(new
			 * Font(Controller.getWindow().getDisplay(),"Calibri", 10,
			 * SWT.None)); GridData gridData = new GridData();
			 * gridData.grabExcessHorizontalSpace = true;
			 * gridData.horizontalAlignment = GridData.CENTER;
			 * noPostAvailable.setLayoutData(gridData); }
			 * 
			 * ((Composite) uiData.get("userPostMaster")).layout(); ((Composite)
			 * uiData.get("userPostMaster")).redraw(); ((Composite)
			 * uiData.get("otherPostWarning")).layout(); ((Composite)
			 * uiData.get("otherPostWarning")).redraw();
			 * Controller.getProfilePanel().getComposite_dinamic().layout();
			 * Controller.getProfilePanel().getComposite_dinamic().redraw();
			 * ((ScrolledComposite)
			 * Controller.getWindow().getParent()).layout();
			 * ((ScrolledComposite)
			 * Controller.getWindow().getParent()).redraw();
			 */
			...
			for (int i = 0; i < posts.length; i++) {
				...
				Display.getCurrent().syncExec(new Runnable() {
					...
					public void run() {
						...
						try {
							labelUserAvatar.setImage(get_ImageStream(new URL(
									posts[j].getUser().Avatar).openStream()));
							labelUserAvatar.setImage(resize(
									labelUserAvatar.getImage(), 75, 75));

						} catch (IOException e) {
							// TODO Auto-generated catch block
							// System.out.println("Eccezione lanciata");
							labelUserAvatar.setImage(get_ImageStream(this
									.getClass()
									.getClassLoader()
									.getResourceAsStream(
			...
			((ProgressBar) uiData.get("pbar")).setVisible(false);
			((ProgressBar) uiData.get("pbar")).setSelection(0);
			// System.out.println("Altezza impostata " +
			// Controller.getWindowHeight() + (150 * ((Composite)
			// uiData.get("userPostMaster")).getChildren().length) );
			// Controller.setScrollHeight(Controller.getWindowHeight() + (250 *
			// ((Composite) uiData.get("userPostMaster")).getChildren().length)
			// );
			...
			((ScrolledComposite) uiData.get("superUserPostMaster")).setMinSize(
					Controller.getWindowWidth() - 50,
					(105 * (((Composite) uiData.get("userPostMaster"))
