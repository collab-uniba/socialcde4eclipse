...
public class ActionRegistrationPanel {
		...
		case "txtMail":
			if (type == SWT.FocusOut) {
				if (InterceptingFilter.verifyMail(((Text) uiData.get("Email"))
					...
					((Label) uiData.get("LabelImageMail")).setData("Image_ok",
							true);
					((Label) uiData.get("LabelImageMail")).setData("Image_no",
							false);
		...
		case "btnRegister":
			...
			if (type == SWT.Selection) {
				...
				case -1:
					...
					/*
					 * Controller.getWindow().layout(); pbWindow.setStop(1);
					 * pbWindow = null;
					 */
					break;
				case 0:
				...
				case 1: // if e-mail address does not exist in the database
					((Label) uiData.get("LabelAlert"))
							.setText("Please insert the email on which you recived the invite");
					((Label) uiData.get("LabelAlert")).setVisible(true);
					/*
					 * Controller.getWindow().layout(); pbWindow.setStop(1);
					 * pbWindow = null;
				...
				case 2:
					...
					((Label) uiData.get("LabelAlert"))
							.setText("Please insert the invitation code that you recived in the invite");
					((Label) uiData.get("LabelAlert")).setVisible(true);
					/*
					 * Controller.getWindow().layout(); pbWindow.setStop(1);
					 * pbWindow = null;
				...
				if (res == 0) {
					...
					Controller.getLoginPanel().inizialize(
							Controller.getWindow());
					/*
					 * pbWindow.setStop(1); pbWindow = null;
					 */
				}
	...
	public int actionRegister(HashMap<String, String> dataExtracted) {
		...
		if (InterceptingFilter.verifyRegistrationPanel(dataExtracted)) {
			...
			/*
			 * Controller.getWindow().layout(); pbWindow.setStop(1); pbWindow =
			 * null;
			 */
		}
