...
public class ActionLoginPanel {
	...
	public ActionLoginPanel(final HashMap<String, Object> uiData) {
		...
		IMAGE_NO = Controller.getLoginPanel().getImageStream(
				this.getClass().getClassLoader()
						.getResourceAsStream("images/no_icon.png"));
		IMAGE_OK = Controller.getLoginPanel().getImageStream(
				this.getClass().getClassLoader()
				...
				if (Controller.getProxy().IsWebServiceRunning()) {
					...
					if (user == null) {
						...
						((Label) uiData.get("labelImageUsername"))
								.setImage(IMAGE_NO);
						((Label) uiData.get("labelImagePassword"))
								.setImage(IMAGE_NO);

						((Label) uiData.get("labelImagePassword")).setData(
								"Image_ok", false);
						((Label) uiData.get("labelImagePassword")).setData(
								"Image_no", true);

						((Label) uiData.get("labelImageUsername")).setData(
								"Image_ok", false);
						((Label) uiData.get("labelImageUsername")).setData(
								"Image_no", true);
					...
					((Label) uiData.get("labelImageHost")).setData("Image_no",
							true);
					((Label) uiData.get("labelImageHost")).setData("Image_ok",
							false);
			...
			try {
				if (type == SWT.Paint) {
					...
					if (!Controller.getPreferences("Autologin").equals("")
							&& Controller.getPreferences("FlagAutologin")
									.equals("True")) {
						...
						if (Controller.getProxy().IsWebServiceRunning()) {
							...
							if (user == null) {
								...
								((Label) uiData.get("labelImageUsername"))
										.setData("Image_no", true);
								((Label) uiData.get("labelImageUsername"))
										.setData("Image_ok", false);
								...
								Controller.setPreferences("Username",
										user.Username);
								Controller
										.setCurrentUserPassword(((Text) uiData
												.get("txtPassword")).getText());

								Controller.setWindowName("Profile");
								Controller.setProfilePanel(new ProfilePanel());

								Controller.getLoginPanel().dispose(
										Controller.getWindow());
								Controller.setLoginPanel(null);
								Controller.getProfilePanel().inizialize(
										Controller.getWindow());
							...
							((Label) uiData.get("labelAlert"))
									.setText("The connection with the Proxy failed");
							((Label) uiData.get("labelImageHost"))
									.setImage(IMAGE_NO);

							((Label) uiData.get("labelImageHost")).setData(
									"Image_no", true);
							((Label) uiData.get("labelImageHost")).setData(
									"Image_ok", false);
