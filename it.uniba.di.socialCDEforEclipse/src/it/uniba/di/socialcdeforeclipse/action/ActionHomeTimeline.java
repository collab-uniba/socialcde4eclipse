...
public class ActionHomeTimeline {
	...
	public ActionHomeTimeline(final HashMap<String, Object> uiData) {
		...
		case "usernameLink":
			...
			WUser userSelected = (WUser) uiData.get("User_data");

			WUser[] userCategory = Controller.getProxy().GetSuggestedFriends(
		...
		case "btnSendMessage":
			...
			if (!InterceptingFilter.verifyText(((Text) uiData
				...
				if (!Controller.getProxy().Post(
					...
					try {
						labelUserAvatar.setImage(get_ImageStream(new URL(
								Controller.getCurrentUser().Avatar)
								.openStream()));
						labelUserAvatar.setImage(resize(
								labelUserAvatar.getImage(), 75, 75));
