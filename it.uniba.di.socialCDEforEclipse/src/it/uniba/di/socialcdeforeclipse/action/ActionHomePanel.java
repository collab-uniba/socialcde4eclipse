			...
			if (type == SWT.Selection) {
				...
				if (service.Registered) {
					...
					if (service.RequireOAuth) {
						// int x = bounds.x +
						// (Controller.getWindow().getShell().getBounds().width
						// - 300) / 2;
						// int y = bounds.y +
						// (Controller.getWindow().getShell().getBounds().height
						// - 200) / 2;
						...
						try {
							PlatformUI
									...
									.getActivePage()
									.showView(
											"it.uniba.di.socialcdeforeclipse.views.SocialCDEviewBrowser");
									...
									serviceSetting
											.setBtnUnsubscriveListener(new Listener() {
												...
												public void handleEvent(
													...
													serviceSetting
															.dispose(null);
													// SquareButtonService.flagDimension
													// = false;
	...
	private void ServiceCancelClick() {
		...
		try {
			...
			try {
				 ...
				 * msgBox.setIcon(SWT.ICON_ERROR); msgBox.start();
				 */
			}
		}
