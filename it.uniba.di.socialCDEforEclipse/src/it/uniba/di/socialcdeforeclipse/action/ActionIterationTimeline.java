...
public class ActionIterationTimeline {
	...
	public ActionIterationTimeline(final HashMap<String, Object> uiData)
	{
		...
		case "otherPostAvailable":
			...
			final WPost[] posts = Controller.getProxy().GetIterationTimeline(Controller.getCurrentUser().Username, Controller.getCurrentUserPassword(),0,getLastId());
			...
			if(posts.length > 0)
			{
			  ...
			  Display.getCurrent().update(); 
				...
				//Controller.setScrollHeight(Controller.getWindowHeight() + (250 * ((Composite)  uiData.get("userPostMaster")).getChildren().length)  );
				 ...
				 WPost[] newPosts = Controller.getProxy().GetIterationTimeline(Controller.getCurrentUser().Username, Controller.getCurrentUserPassword(),0,getLastId());
				 System.out.println("Nuovi Post trovati " + newPosts.length + " con id " + getLastId()); 
