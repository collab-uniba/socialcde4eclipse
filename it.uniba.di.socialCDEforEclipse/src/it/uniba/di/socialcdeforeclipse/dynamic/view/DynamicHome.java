...
public class DynamicHome implements Panel {
	...
	public void inizialize(Composite panel) {
		...
		// System.out.println("servicecomposite size " +
		// serviceComposite.getBounds());
		// serviceComposite.setSize(serviceComposite.computeSize(serviceComposite.getSize().x,
		// 100 * (wService.length/2)));
		// System.out.println("servicecomposite size after " +
		// serviceComposite.getBounds());
				...
				Display.getCurrent().syncExec(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							services.setImage(get_ImageStream(new URL(
									Controller.getPreferences("ProxyRoot")
											+ wService[j].Image).openStream()));
